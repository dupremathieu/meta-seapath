...
- **Layer**: `meta-seapath`, compatible with `scarthgap` (Yocto 5.0), depends on `efi-secure-boot`
...
- **Push** (`push.yml`): dispatches to main repo's `push.yml` on push to `scarthgap` branch.
- **Manual build** (`ci-build-yocto.yaml`): full Yocto build, runs on self-hosted `seapath-yocto-builder` runner.

Branch: `scarthgap` is the primary branch (not `main`/`master`).
